...
func _handle_message(p_message: ConstaNetHeadder, p_source: StreamPeerTCP = null) -> void:
	match p_message.type:
		MessageType.DISCOVERY:
			_handle_discovery(p_message, p_source)
...
func _handle_discovery(p_discovery: ConstaNetDiscovery, p_source: StreamPeerTCP = null) -> void:
	_update_from_discovery(p_discovery)
	print(p_discovery)
	if is_instance_valid(p_source):
		if p_discovery.is_request():
			_use_stream(p_source)
			_send_tcp_discovery_acknowledment()
			_set_connection_status(NetworkNode.ConnectionState.CONNECTED)
		
		elif p_discovery.is_acknowledgment() and _connection_state == ConnectionState.CONNECTING:
			_set_connection_status(NetworkNode.ConnectionState.CONNECTED)
