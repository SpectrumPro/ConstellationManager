...
## Returns a human readable realtive time. ie "2 days ago", or "in 3 weeks"
static func relative_time(p_timestamp: int) -> String:
	var delta: int = int(Time.get_unix_time_from_system()) - p_timestamp
	var is_future: bool = delta < 0
	
	var amount: int
	var unit: String
	
	var intervals: Array[Array] = [
		[60, "second"],
		[3600, "minute"],
		[86400, "hour"],
		[604800, "day"],
	...
	if delta < 60:
		...
		for i in range(intervals.size() - 1):
			if delta < intervals[i + 1][0]:
				amount = int(delta / intervals[i][0])
				unit = intervals[i][1]
				break
		if amount == 0:
			amount = int(delta / 31536000)
			unit = "year"

	var label: String = "%d %s%s" % [amount, unit, "s" if amount != 1 else ""]
	return ("in " + label) if is_future else (label + " ago")
